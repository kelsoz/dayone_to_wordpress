...
def png_jpg(file_path):
    ...
    if (x >MAXSIZEX):
        ...
        img = img.resize((x,y), Image.ANTIALIAS)
        img.convert("RGB").save(file_path,"png",quality = 95)


def resize_judge(file_path):
    img = Image.open(file_path)
    x,y =  img.size
    if x >2100:
        print "need scale"
...
def resize_img(filepath):
    ...
    if os.path.exists(filepath) and resize_judge(filepath):
            ex = os.path.splitext(filepath)[1]
            print "ex:",ex

            if ex == ".tif" or ex == ".tiff" or ex == ".TIF" or ex == ".TIFF" or ex == ".jpg" or ex == ".JPG":
                print "is a tiff file"
                tif_jpg(filepath)
            elif ex == ".png" or ex == ".PNG":
                print "png file"
                png_jpg(filepath)
            else:
                print "不支持的文类型"

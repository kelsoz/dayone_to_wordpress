...
import json,sys,mistune,os,md5,datetime
...
def new_post_with_pic(content_raw,photos_info_list,post_date):
    ...
    photos_info_list = add_photos_path(photos_info_list,photos_path)
    print photos_info_list

    print "上传photo,只上传必要图片,写入key"
...
def print_photos_list(photos_list):
    for photo in photos_list:
        for i in photo: 
            print "%s=" % i,photo[i]
        print "\n\n"

def add_photos_path(photos_list,photos_path):
    """
    photos_list数组里元素格式(dict):
    ...
    # print "finder_photos:",finder_photos
    for img in photos_list:
        img[u"url"] = False
        img[u"file_path"] = u"这里是路径"
        img_md5 = img[u"md5"]

        for finder_photo in finder_photos:
            if img_md5 == finder_photo[u"md5"]:
                img[u"file_path"] = finder_photo[u"file_path"]
...
def upload_pic_list(pic_list):
    ...
    for pic in pic_list:
        if pic[u"url"] == False:
            pic_url,pic_id = upload_pic(pic[u"file_path"],)
            pic[u"url"] = pic_url
            pic[u"id"] = pic_id
            n = n + 1
...
def downlaod_pic(picurl):
    import urllib2

    save_path= os.environ['HOME'] + "/Downloads/"
    img_name = urllib2.unquote(picurl).decode('utf8').split('/')[-1]
    print "准备下载图片:",img_name
    try: 
        ...
        while os.path.exists(fileName):
            fileName = save_path + str(n) + "_" + img_name
            n = n + 1
        ...
        print "DOWNLOAD %s ERROR!==>>%s" % (picurl, error) 
        return False
    except Exception, e: 
        return False
...
def finder_vs_wp_photos(finder_img_list,wp_list):
    # """
    # wp_img_list数组里元素格式(dict):
    # "url":img["img_url"] , 
    # "path":img_path,
    # "md5":getmd5(img_path),
    # "id":img["id"]
    ...
    # "file_path": /kelso/....
    # """
    wp_img_list = wp_list[:] #传参list是个大坑
    not_del_list = []

    for img in finder_img_list:
        # print "本地图片md5:", img["md5"] 
        for wp_img in wp_img_list:
            # print "wp图片md5:", wp_img["md5"]
            if img["md5"] == wp_img["md5"]:
                # print "此图片已存在wp：",img['file_path']
                img['url'] = wp_img["url"]
                img['id'] = wp_img['id']
                not_del_list.append(wp_img)

    for i in not_del_list:
        wp_img_list.remove(i)
        #删除有用的图片信息,只剩下无用的wp图片

    del_post_img_out(wp_img_list)
    return finder_img_list
